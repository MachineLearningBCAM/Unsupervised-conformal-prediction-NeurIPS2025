...
Aeq=kron(ones(1,Y),eye(n_cal));
  ...
  if mosek==0 
         options=optimoptions('quadprog','Display','off','LinearSolver','auto','Algorithm','interior-point-convex','OptimalityTolerance', 1e-8, ...
...
tic
    % Scaling the objective by n_cal leaves its optimizer unchanged and
    % matches the CVX branch; it also improves quadprog's stopping accuracy.
    p=quadprog(sparse(2*Kc/n_cal),sparse(-2*v/n_train),sparse(Aloss),sparse(upper_loss),sparse(Aeq),sparse(beq),lb,ub,[],options);
time=toc;
